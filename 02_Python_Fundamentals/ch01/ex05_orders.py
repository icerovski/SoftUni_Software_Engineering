...
integer N - the number of orders the shop will receive. For each order, you will
...
you should ignore it and move to the next one. After you go through all orders,
you need to print the total price in the following format:
•	 "Total: ${total_price}"
...
for _ in range(0, NUM):
    ...
    if (0.01 <= price_per_capsule <= 100 and
        1 <= days <= 31 and 1 <= capsules <= 2000):

        cost_per_capsule = price_per_capsule * days * capsules
        total_cost += cost_per_capsule
        print(f'The price for the coffee is: ${cost_per_capsule:.2f}')
